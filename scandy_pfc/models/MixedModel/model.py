...
class MixedModel(Model):
    """
    Object-based model with location-based IOR: potential saccade targets are object files.
    """

    name = "mix"
    description = "Object-based model with location-based IOR."
    ...
    def __init__(self, Dataset, params=None, preload_res_df=None):
        ...
        self._ior_map = np.zeros_like(self._feature_map) # !!! different from the object model!!!
    ...
    def load_default_modelparams(self):
        ...
        # this is the most important difference to the object model
        par.ior_dva = 2.0
    ...
    def reinit_for_sgl_run(self):
        ...
        self._ior_map = np.zeros_like(self._ior_map) # !!! different from the object model!!!
    ...
    def update_decision(self):
        ...
        for obj_id, obj in enumerate(self.video_data["object_list"]):
            self._decision_objs[obj_id] = obj.update_evidence_locior(
                self._current_frame,
                self._cur_fov_frac,
                self._feature_map,
                self._sens_map,
