...
class ObjectFile:
    ...
    def update_foveation(self, frame, gaze_loc):
        ...
        # Set detected to true when an object is foveated for the first time!
        if self.foveated and not self.ground:
            self.detected = True
    ...
    def update_ior(self, model_params):
        ...
        if self.foveated:
            ...
            self._ior_tempmem = model_params["ior_max"]  # 1
            if self.ground:
                self.ior = 0.0
            else:
            ...
            if self._ior_tempmem > 0:
                ...
                # TODO alternatively set self.detected here?
            self.ior = self._ior_tempmem * 1.0  # set to same value but not make them the same object
        return self.ior

    def update_evidence(self, frame, fov_frac, feature_map, sens_map, topdown_map, model_params):
        """
        ...
        if self.appears_in[frame]:
            masked_feature_map = feature_map[self.object_maps[frame]]
