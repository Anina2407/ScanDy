...
class ObjectModel(Model):
    ...
    def __init__(self, Dataset, params=None, preload_res_df=None):
        ...
        default_params = self.load_default_modelparams()

        # If user provided params, update defaults
        if params is not None:
            # Only override keys that exist in params
        ...
        self._topdown_map = np.zeros_like(self._feature_map)
    ...
    def load_default_modelparams(self):
        ...
        par.startpos = "center"
        # Max IOR value (was previously always assumed as 1, meaning full inhibition)
        par.ior_max = 1.0
        ...
        par.topdown_std = 1.0
        par.featurescale = [0,0]
        # alternatively to scaling between 0-1 can also be scale set to mean=1 and std=?
        par.feature_std = 0.0
        par.feature_fade = False
    ...
    def reinit_for_sgl_run(self):
        ...
        self._feature_map = np.zeros_like(self._feature_map)
        self._topdown_map = np.zeros_like(self._topdown_map)
        self._sens_map = np.zeros_like(self._sens_map)
    ...
    def update_features(self):
        ...
        if self.params.get("topdown_mode",0) == "detected_objects":
            self._topdown_map = self.video_data["topdown_maps"][self._current_frame]
        else:
    ...
    def update_sensitivity(self):
        ...
        if self.params.get("sglrun_return", False):
