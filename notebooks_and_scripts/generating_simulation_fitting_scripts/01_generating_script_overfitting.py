...
parser.add_argument('--std', type=float, nargs='+', default=[0.5, 0.2], help='List of std values without comma')
parser.add_argument('--trainlist_size', type=int, default=10, help='Training list size')
parser.add_argument('--frames', type=int, default=150, choices=[90, 150], help='Number of frames (with 30 FPS)')
parser.add_argument('--best_range', type=int, default=3, help='Number of best parameters to simulate')
parser.add_argument('--max_seed', type=int, default=13, help='Maximum seed value')
parser.add_argument('--evolution_name', type=str, default="overfitting/evol_not05_", help='Evolution name prefix')
parser.add_argument('--simulate_name', type=str, default="overfitting/best_not05_", help='Simulation name prefix')
...
for std_value in std_list:
    for test_group in ["young"]:
        ...
        def optimize_me(traj):
            ...
            sim_fovcat_ratio = np.array(list(model.get_fovcat_ratio("train").values()))
            # Mean squared error RMSE (sqrt(mean(squared_difference))) 
            bdir_ratio_error = np.sqrt(np.mean((gt_fovcat_ratio - sim_fovcat_ratio)**2))

            # 2. BDIR timecourse fitness (unweighted)
            sim_bdir_timecourse = get_BDIR_per_frames(model.result_df, fps=30, max_frames=frames)

            # Mean squared error RMSE (sqrt(mean(squared_difference))) across all BDIR categories and frames
            timecourse_error_square = (gt_bdir_timecourse - sim_bdir_timecourse)**2
            timecourse_error = np.sqrt(np.mean(timecourse_error_square)) / 100
            
            # # 2. BDIR timecourse fitness (weight first frames more)
            # tau = 30
            ...
            # print(f"Frame 0 weight: {time_weights[0]:.2f}")    # Should be ~2.5
            # print(f"Frame 30 weight: {time_weights[30]:.2f}")  # Should be ~1.3
            # print(f"Frame 120 weight: {time_weights[120]:.2f}") # Should be ~0.6
            
            # Fitness tuple: (duration KS, amplitude KS, BDIR error)
            fitness_tuple = (ks_dur, ks_amp, bdir_ratio_error, timecourse_error)
        ...
        # weight list, emphasizing BDIR components more
        weight_list = [-0.5, -0.75, -2, -2.5]  
        ...
        for rang in range(best_range):
          ...
          # Adding Penalizing in optimize function #
           # Define targets for each component
            # TARGET_KS_DUR = 0.15       # Target for duration KS statistic
            # TARGET_KS_AMP = 0.15       # Target for amplitude KS statistic  
            # TARGET_BDIR_RATIO = 0.10   # Target for BDIR ratio error (TVD)
            # TARGET_BDIR_TIME = 0.05    # Target for BDIR timecourse error

            # PENALTY_WEIGHT = 10.0      # How aggressively to penalize

            # # Calculate penalties (only penalize if above target)
            # penalty_dur = max(0, ks_dur - TARGET_KS_DUR) ** 2
            # penalty_amp = max(0, ks_amp - TARGET_KS_AMP) ** 2
            # penalty_bdir_ratio = max(0, bdir_ratio_error - TARGET_BDIR_RATIO) ** 2
            # penalty_bdir_time = max(0, timecourse_error - TARGET_BDIR_TIME) ** 2

            # # Apply penalties to fitness
            # ks_dur_penalized = ks_dur + PENALTY_WEIGHT * penalty_dur
            # ks_amp_penalized = ks_amp + PENALTY_WEIGHT * penalty_amp
            # bdir_ratio_penalized = bdir_ratio_error + PENALTY_WEIGHT * penalty_bdir_ratio
            # bdir_time_penalized = timecourse_error + PENALTY_WEIGHT * penalty_bdir_time

            # Fitness tuple with 4 separate objectives
            # fitness_tuple = (ks_dur_penalized, ks_amp_penalized, 
            #                 bdir_ratio_penalized, bdir_time_penalized)  
            
            # Print penalty information for first few evaluations
            # if not hasattr(optimize_me, '_penalty_print_count'):
            #     optimize_me._penalty_print_count = 0
            
            # if optimize_me._penalty_print_count < 5:
            #     print(f"\n--- Penalty Analysis ---")
            #     print(f"Raw values: ks_dur={ks_dur:.4f}, ks_amp={ks_amp:.4f}, bdir_ratio={bdir_ratio_error:.4f}, bdir_time={timecourse_error:.4f}")
            #     print(f"Penalties: dur={PENALTY_WEIGHT * penalty_dur:.4f}, amp={PENALTY_WEIGHT * penalty_amp:.4f}, bdir={PENALTY_WEIGHT * penalty_bdir_ratio:.4f}")
            #     print(f"Penalized: ks_dur={ks_dur_penalized:.4f}, ks_amp={ks_amp_penalized:.4f}, bdir_ratio={bdir_ratio_penalized:.4f},  bdir_time={bdir_time_penalized:.4f}")
            #     optimize_me._penalty_print_count += 1